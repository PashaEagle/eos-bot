...
@Service(value = CartKeyboard.BEAN_NAME)
@Scope(value = BeanDefinition.SCOPE_PROTOTYPE)
public class CartKeyboard extends BaseCartKeyboard {

    public static final String BEAN_NAME = "cartKeyboard";

    private final Bot bot;

    public CartKeyboard(Bot bot) {
        this.bot = bot;
    }
    ...
    public void fillMenuRows(Update update) {
        ...
        if (!items.isEmpty()) {
            ...
            if (session.getProfile().getAddress() != null) {
                Map<String, String> fourthRow = new HashMap<>();
