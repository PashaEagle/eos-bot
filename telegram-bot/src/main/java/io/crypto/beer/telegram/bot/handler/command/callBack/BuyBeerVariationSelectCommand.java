...
public class BuyBeerVariationSelectCommand implements ICommandCallBack {
    ...
    public void makeAction(Update update) {
        ...
        List<Variation> variations = session.getShopData().getVariations();
        variations.stream().filter(v -> v.getName().equals(message)).findFirst()
                .ifPresent(session.getShopData()::setVariation);
    ...
    public boolean check(Update update) {
        if (update.hasCallbackQuery()) {
            ...
            return command.equals(CallBackConstants.BTN_TXT_VARIATION_SELECT_CALL_BACK);
        }
        return false;
    }
